...
export default function MainContent() {
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: { xs: 'column-reverse', md: 'row' }, justifyContent: 'space-between', gap: 4 }}>
                ...
                {cardData.map((card, index) => (
                    <Grid item xs={12} md={index < 2 ? 6 : 4} key={index}>
                        <StyledCard tabIndex={0}>
                            {index !== 3 && index !== 4 ? (
                                <CardMedia component="img" image={card.img} alt={card.title} sx={{ aspectRatio: '16 / 9', borderBottom: '1px solid', borderColor: 'divider' }} />
                            ) : null}
